  ...
  return (
    <main style={{minHeight:"100vh",background:"radial-gradient(circle at top, #123047 0%, #060b18 45%, #02040a 100%)",color:"white",padding:"32px 20px 56px",boxSizing:"border-box"}}>
      <div style={{width:"100%",maxWidth:920,margin:"0 auto"}}>
        {!enhancedGravity && <Link href="/" style={{display:"inline-block",color:"white",textDecoration:"none",marginBottom:30,fontSize:13,fontWeight:800,opacity:.8}}>← {text.page.home}</Link>}
        {!enhancedGravity && <>
        ...
        {enhancedGravity ? <GravityPlayground onBack={()=>setEnhancedGravity(false)} /> : <div onClickCapture={handleLaboratoryClick}><LocalizedLaboratory /></div>}
      </div>
    </main>
  );
}
